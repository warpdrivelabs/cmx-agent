...
impl Agent {
    ...
    pub async fn run_turn_observed_as(
        ...
        let subject = turn_subject.unwrap_or(&self.policy.subject);
        ...
        let reason = loop {
            ...
            self.handle_tool_calls_as(session, &resp.tool_calls, subject).await;
        };
    ...
    /// 守卫按 `subject` 判定（IM 绑定：绑定用户身份过守卫，与桌面登录主体 policy.subject 并发无竞态）。
