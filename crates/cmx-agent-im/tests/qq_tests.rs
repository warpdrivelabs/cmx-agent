...
fn temp_app(model: MockModel) -> Arc<AgentApp> {
    let n = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let base = std::env::temp_dir().join(format!("cmx-qq-test-{n}"));
...
async fn qq_bridge_blocks_unauthorized() {
    ...
    let evs = app.get_events("im-qq-usr_999").unwrap_or_default();
    assert!(evs.is_empty(), "未授权 chat 不应建会话");
}

#[tokio::test]
async fn qq_same_chat_reuses_session_with_qq_prefix() {
...
async fn qq_binding_mode_unbound_sender_gets_prompt_no_turn() {
    ...
    let evs = app.get_events("im-qq-usr_1").unwrap_or_default();
...
async fn qq_binding_mode_bound_sender_runs_turn() {
    ...
    let evs = app.get_events("im-qq-usr_2").unwrap_or_default();
